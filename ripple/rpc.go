...
func (rpc RippleRPC) GetCurrentIndex() (uint64, error) {
	...
	respData, err = Call(`{"method": "ledger_current", "params":[]}`, rpc.url)
	if err != nil {
		return 0, err
	}
...
func (rpc RippleRPC) GetBlockByIndex(h uint64, isTx bool) ([]byte, error) {
	...
	blockData, err = Call(`{"method": "ledger", "params":[`+string(reqBytes)+`]}`, rpc.url)
...
func (rpc RippleRPC) GetRawTransaction(h string) ([]byte, error) {
	...
	tx, err = Call(`{"method": "tx", "params":[`+string(reqBytes)+`]}`, rpc.url)
	return tx, err
}
...
func (rpc RippleRPC) GetAccountTxs(a string, minIndex uint64) ([]byte, error) {
	...
	return Call(`{"method":"account_tx", "params":[`+string(reqBytes)+`]}`, rpc.url)
}
